...
from modules.app_translation import Translator as CustomTranslator
...
class aclient(discord.AutoShardedClient):
        ...
        def get_activity() -> discord.Activity:
            ...
            if activity_type == "Playing":
                ...
                return discord.Activity(
                    type=discord.ActivityType.listening, name=activity_title
                )
            elif activity_type == "Watching":
                return discord.Activity(
                    type=discord.ActivityType.watching, name=activity_title
                )
            elif activity_type == "Competing":
                return discord.Activity(
                    type=discord.ActivityType.competing, name=activity_title
                )
    ...
    async def on_app_command_error(
        ...
        if isinstance(error, discord.app_commands.CommandOnCooldown):
            ...
            try:
                try:
                    await interaction.response.send_message(
                        "Error! Try again.", ephemeral=True
                    )
                except:
                    try:
    ...
    async def on_ready(self) -> None:
        await bot.change_presence(
            activity=self.Presence.get_activity(), status=self.Presence.get_status()
        )
        if self.initialized:
            return
        ...
        program_logger.info(
            f"Initialization completed in {time.time() - startupTime_start} seconds."
        )
...
# Load Modules
from modules import context_commands, nickname
...
class Owner:
    async def log(message, args) -> None:
        ...
        await message.channel.send(
            content=f"Here are the last {len(log_lines)} lines of the current logfile:",
            file=discord.File(buffer_file_path),
        )
    ...
    async def activity(message, args) -> None:
        ...
        await bot.change_presence(
            activity=bot.Presence.get_activity(), status=bot.Presence.get_status()
        )
        await message.channel.send(
            f'Activity set to {action} {title}{" " + url if url else ""}.'
        )
...
async def cmd_ping(interaction: discord.Interaction) -> None:
    ...
    gateway_ping = (
        ...
        else bot.shards.get(interaction.guild.shard_id).latency * 1000
    )
...
async def cmd_botinfo(interaction: discord.Interaction) -> None:
    ...
    embed = discord.Embed(
        title=f"Information about {bot.user.name}", color=discord.Color.blue()
    )
    ...
    embed_fields = [
        ...
        (
            "Shard ID",
            f"{interaction.guild.shard_id if interaction.guild else 'N/A'}",
            True,
        ),
        ("\u200b", "\u200b", True),
        ...
        (
            "Invite",
            f"[Invite me](https://discord.com/api/oauth2/authorize?client_id={bot.user.id})",
            True,
        ),
        ("\u200b", "\u200b", True),
    ]
...
async def cmd_support(interaction: discord.Interaction) -> None:
    ...
    if not SUPPORTID:
        await interaction.followup.send(
            "There is no support server setup!", ephemeral=True
        )
        return
    if interaction.guild is None:
        await interaction.followup.send(
            await Functions.create_support_invite(interaction), ephemeral=True
        )
        return
    if str(interaction.guild.id) != SUPPORTID:
        ...
        await interaction.followup.send(
            "You are already in our support server!", ephemeral=True
        )
